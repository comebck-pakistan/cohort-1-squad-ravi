"""replies.py - randomized question bank + niche-aware preferences picker + edit replies."""
import random
import re
...
ALREADY_REGISTERED_REPLY = (
    "You're already registered with our platform! We'll reach out as soon as there's a match. "
    "(Type 'reset ai' if you'd like to start over)"
)

RESET_REPLY = "Done! Your data's been wiped. Send 'Hi' whenever you're ready to start fresh. 👋"
...
def _detect_niche(temp_data: dict | None) -> str | None:
    """Scans text fields in temp_data for keyword hits and returns the matching
    niche name, or None if nothing matches."""
    if not temp_data:
        return None

    searchable = " ".join(
        str(v) for v in [
            temp_data.get("project_description"),
            temp_data.get("brief_description"),
            temp_data.get("skills"),
            temp_data.get("tools"),
            temp_data.get("name"),
        ] if v
    ).lower()
...
def pick_preferences_reply(temp_data: dict | None) -> str:
    """Smart preferences reply picker: keyword-match temp_data -> pick a random
    message from that niche's bank; no match -> pick from ALL variants."""
    niche = _detect_niche(temp_data)

    if niche:
        matched = next((v for v in PREFERENCE_VARIANTS if v["niche"] == niche), None)
        if matched:
            return _pick_random(matched["messages"])

    all_messages = [m for v in PREFERENCE_VARIANTS for m in v["messages"]]
    return _pick_random(all_messages)


def pick_reply_text(next_step: str | None) -> dict:
    step = (next_step or "welcome").lower().strip()
    if "completed" in step:
        step = "completed"
    items = QUESTIONS.get(step, QUESTIONS["welcome"])
    return {"step": step, "text": _pick_random(items)}


def pick_post_completion_reply() -> str:
    return _pick_random(POST_COMPLETION_REPLIES)


def get_already_registered_reply() -> str:
    return ALREADY_REGISTERED_REPLY


def get_reset_reply() -> str:
    return RESET_REPLY


def is_skip_message(text: str | None) -> bool:
    if not text:
        return False
    t = text.strip().lower()
    if t == "skip":
        return True
    return any(phrase in t for phrase in ["skip this", "no thanks", "n/a", "none", "pass"])


def _format_current_data(data: dict | None) -> str:
    if not data:
        return "Nothing saved yet!"
    lines = [f"- *{k}*: {v if v else 'Not provided'}" for k, v in data.items()]
    return "\n".join(lines) if lines else "Nothing saved yet!"


def pick_edit_vague_reply(current_data: dict | None) -> str:
    text = _pick_random(EDIT_VAGUE_REPLIES)
    return text.replace("{LIST}", _format_current_data(current_data))


def pick_edit_confirm_reply(field: str) -> str:
    text = _pick_random(EDIT_SPECIFIC_CONFIRM_REPLIES)
    return re.sub(r"\{FIELD\}", field.replace("_", " "), text)


def pick_edit_success_reply(field: str, value) -> str:
    text = _pick_random(EDIT_SUCCESS_REPLIES)
    text = re.sub(r"\{FIELD\}", field.replace("_", " "), text)
    text = re.sub(r"\{VALUE\}", str(value) if value else "Not provided", text)
    return text
